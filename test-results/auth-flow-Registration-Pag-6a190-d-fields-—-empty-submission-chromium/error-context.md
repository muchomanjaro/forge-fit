...
- Name: auth-flow.spec.ts >> Registration Page >> should validate required fields — empty submission
- Location: e2e/auth-flow.spec.ts:66:7
...
> 67  |     await page.goto('/auth/register');
  ...
  150 | // ─── Login Page ──────────────────────────────────────────────────
  151 | 
  152 | test.describe('Login Page', () => {
  153 |   test('should load login form', async ({ page }) => {
  154 |     await page.goto('/auth/login');
  155 |     await expect(page.locator('text=Welcome back')).toBeVisible();
  156 |     await expect(page.locator('text=Log in to your account')).toBeVisible();
  157 |     await expect(page.locator('#email')).toBeVisible();
  158 |     await expect(page.locator('#password')).toBeVisible();
  159 |   });
  160 | 
  161 |   test('should display Google OAuth button on login', async ({ page }) => {
  162 |     await page.goto('/auth/login');
  163 |     const googleBtn = page.locator('button:has(svg):has-text("Continue with Google")');
  164 |     await expect(googleBtn).toBeVisible();
  165 |   });
  166 | 
  167 |   test('should have link to sign up page', async ({ page }) => {
```